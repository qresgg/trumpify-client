import styles from "../styles/SelectionSlider.module.scss";
import { useEffect, useState } from "react";
import Skeleton from "react-loading-skeleton";
import { AlbumSelectionItem  } from "../items/album.selection.item";
import { SongSelectionItem  } from "../items/song.selection.item";
...
export function Selection({ title, fetchFunction, id = null }) {
    ...
    const renderItem = (item, type, index) => {
      switch (type) {
        case "Album":
          return <AlbumSelectionItem key={index} item={item} />
        case "Song":
          return <SongSelectionItem key={index} item={item} />
        default:
          return null;
      }
    };
      ...
      <div className={styles['selections']}>
        <div className={styles['selections__container']}>
          <div className={styles['selections__header']}>
              <div className={styles['selections__title']}>{title}</div>
              {/* <div className={styles.showall}>Show all</div> */}
          </div>
          <div className={styles['selections__content']} ref={containerRef}>
              {loading 
                ? <div className={styles['selections__skeleton']}>
                  {Array.from({ length: settings.slidesToShow }).map((_, index) => (
                  <div key={index} className={styles['selections__skeletonItem']}>
